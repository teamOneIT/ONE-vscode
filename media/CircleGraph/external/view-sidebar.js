...
const ActivationFunctionType = ['NONE', 'RELU', 'RELU_N1_TO_1', 'RELU6', 'TANH', 'SIGN_BIT'];
const Padding = ['SAME', 'VALID'];
...
sidebar.NodeSidebar = class {
    ...
    _modify(modifyButton, saveButton, cancelButton) {
        ...
        line.className = 'line';
        attributesBox.appendChild(line);
        for (const attribute of sortedAttributes) {
    ...
    constructor(host, attribute) {
        ...
        switch (type) {
            ...
            default: {
                ...
                let line;
                if(attribute['_type'] === 'ActivationFunctionType'){
                    line = this._host.document.createElement('select');
                    for(let options of ActivationFunctionType){
                    ...
                    line.setAttribute('name', 'ActivationFunctionType');
                    line.className = 'sidebar-view-item-value-line-select';
                }
                else if(attribute['_type'] === 'Padding'){
                    line = this._host.document.createElement('select');
                    for(let options of Padding){
                        const option = this._host.document.createElement('option');
                        option.setAttribute('value', options);
                        option.innerText = options;
                        if(options === content){
